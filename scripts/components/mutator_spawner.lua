-- chunkname: @scripts/components/mutator_spawner.lua
...
MutatorSpawner.enable = function (self, unit)
	return
end

MutatorSpawner.disable = function (self, unit)
	return
end

MutatorSpawner.editor_init = function (self, unit)
	...
	if MutatorSpawner._nav_info == nil then
		MutatorSpawner._nav_info = SharedNav.create_nav_info()
	end
...
MutatorSpawner.editor_validate = function (self, unit)
	local success = true
	local error_message = ""
...
MutatorSpawner.editor_destroy = function (self, unit)
	if not rawget(_G, "LevelEditor") then
		return
	end
	...
	if MutatorSpawner._nav_info ~= nil then
		SharedNav.destroy(MutatorSpawner._nav_info)
	end
...
MutatorSpawner.editor_update = function (self, unit)
	if not rawget(_G, "LevelEditor") then
		return
	end

	local with_traverse_logic = false
	local nav_gen_guid = SharedNav.check_new_navmesh_generated(MutatorSpawner._nav_info, self._my_nav_gen_guid, with_traverse_logic)
...
MutatorSpawner.editor_on_mission_changed = function (self, unit)
	if not rawget(_G, "LevelEditor") then
		return
	end
...
MutatorSpawner.editor_world_transform_modified = function (self, unit)
	if not rawget(_G, "LevelEditor") then
		return
	end
...
MutatorSpawner.editor_property_changed = function (self, unit)
	if not rawget(_G, "LevelEditor") then
		return
	end
...
MutatorSpawner.editor_selection_changed = function (self, unit, selected)
	self._selected = selected

	self:_refresh_debug_draw(unit)
end

MutatorSpawner._refresh_debug_draw = function (self, unit)
	local gui = self._gui
...
MutatorSpawner._debug_draw_main_path_connection = function (self, unit, drawer)
	local gui = self._gui
	...
	if is_main_path_registered then
		...
		local nav_world = MutatorSpawner._nav_info.nav_world_from_level_id[active_mission_level_id]
...
MutatorSpawner._debug_draw_witch_spawner = function (self, unit, drawer)
...
MutatorSpawner.editor_toggle_debug_draw = function (self, enable)
	if not rawget(_G, "LevelEditor") then
		return
	end
