-- chunkname: @scripts/components/vector_field_swirl.lua

local VectorFieldSwirl = component("VectorFieldSwirl")

VectorFieldSwirl.init = function (self, unit)
	self._effect = self:get_data(unit, "effect_resource_01")
	self._unit = unit
	self._whirl_speed = self:get_data(unit, "whirl_speed")
...
VectorFieldSwirl.editor_validate = function (self, unit)
	return true, ""
end

VectorFieldSwirl.enable = function (self, unit)
...
VectorFieldSwirl.update_direction = function (self, unit)
	if self._whirl_speed < 0 then
...
VectorFieldSwirl.update_bounding_volume = function (self, unit)
	if self._bounding_volume == "none" then
		Unit.set_visibility(unit, "bounding", false)
	end
end

VectorFieldSwirl.disable = function (self, unit)
	self:remove(unit)
end

VectorFieldSwirl.destroy = function (self, unit)
	self:remove(unit)
end

VectorFieldSwirl.changed = function (self, unit)
	...
	if vector_field_id then
		...
		Log.info("VectorFieldSwirl", "No Vectorfield ID in Unit")
	end
end

VectorFieldSwirl.remove = function (self, unit)
	local vector_field_id = Unit.get_data(unit, "vector_field_id")
...
VectorFieldSwirl.start_effect = function (self)
	self:disable(self._unit)
	self:enable(self._unit)
end

VectorFieldSwirl.stop_effect = function (self)
	self:disable(self._unit)
end

VectorFieldSwirl.create_paramaters = function (self, unit)
...
VectorFieldSwirl.component_data = {
	vector_field_name = {
