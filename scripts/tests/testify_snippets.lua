local TestifySnippets = {
	...
	load_first_mission_from_mission_board_on_peer = function (peer_id)
	...
	load_mission_in_mission_board = function (level_key, challenge, resistance, circumstance_name, side_mission)
	...
	set_free_flight_camera_position = function (camera_data)
		if DEDICATED_SERVER then
	...
	activate_debug_function = function (debug_function, params, peer_id)
		local debug = {
	...
	debug_function_first_option = function (debug_function, peer_id)
		if peer_id then
	...
	wait_for_peers = function (num_peers)
		while Testify:num_peers() ~= num_peers do
			coroutine.yield()
		end
	end,
	send_request_to_all_peers = function (request_name, wait_for_response, num_retries, ...)
	...
	check_flags_for_mission = function (flags, mission_key)
		for i = 1, #flags, 1 do
			local flag = flags[i]
	...
	mission_flag_of_type = function (mission, flag_type)
	...
	trigger_vo_query_player_look_at = function (look_at_tag, distance, num_dialogues)
		local look_at_data = {
	...
	trigger_vo_query_faction_look_at = function (faction, look_at_tag, distance, num_dialogues)
		local look_at_data = {
			faction = faction,
			look_at_tag = look_at_tag,
			distance = distance
	...
	trigger_vo_query_mission_brief = function (mission_brief_starter_line, voice_profile, num_dialogues)
	...
	trigger_vo_query_mission_giver_mission_info = function (trigger_id, voice_profile, num_dialogues)
	...
	trigger_vo_query_player_generic_vo = function (trigger_id, num_dialogues)
	...
	trigger_vo_query_player_environmental_story_vo = function (trigger_id, num_dialogues)
	...
	trigger_mission_giver_conversation_starter = function (trigger_id, voice_profile, num_dialogues)
	...
	trigger_vo_query_player_start_banter = function (trigger_id, num_dialogues)
	...
	trigger_vo_on_demand = function (vo_concept, trigger_id, num_dialogues)
	...
	wait_for_end_of_dialogue = function (num_dialogues)
		local has_played = true
		local SOUND_MINIMUM_TIME = 0.1

		for i = 1, num_dialogues, 1 do
	...
	equip_all_traits_support_snippet = function (player, slot_name, traits, has_placeholder_profile, weapon_name, units_to_spawn)
	...
	reset_weapon_traits = function (player, slot_name)
	...
	wait_for_gameplay_ready = function ()
		if not DEDICATED_SERVER then
	...
	connection_statistics = function ()
