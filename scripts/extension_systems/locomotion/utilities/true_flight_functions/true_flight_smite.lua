...
true_flight_smite.smite_update_towards_position = function (target_position, physics_world, integration_data, dt, t, optional_validate_impact_func, optional_on_impact_func)
	...
	local current_off = math.abs(1 - Vector3.dot(current_direction, wanted_direction))
	local is_aligned_offset = true_flight_template.is_aligned_offset
	local current_is_aligned = current_off < is_aligned_offset
	...
	if current_is_aligned then
		...
		lerp_modifier = lerp_modifier_1 * lerp_modifier_1 * math.min(time_without_bounce, 0.25) / 0.25
	else
		local lerp_modifier_1 = math.min(time_without_bounce, 1) / 1
	...
	new_position, new_velocity = ProjectileLocomotionUtility.impact_detection_and_resolution(integration_data, new_position, new_velocity, physics_world, collision_filter, dt, false, skip_statics)
