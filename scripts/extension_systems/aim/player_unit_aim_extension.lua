...
PlayerUnitAimExtension.update = function (self, unit, dt, t)
	...
	local target_position = root_position + height * Vector3.up() + direction * self._aim_contraint_distance

	fassert(Unit.animation_has_constraint_target(unit, self._aim_constraint_target_name), "[PlayerUnitAimExtension][update] Animation does not have any constraint targets.")
