-- chunkname: @scripts/extension_systems/smart_targeting/precision_target_finder.lua
...
local PrecisionTargetFinder = class("PrecisionTargetFinder")

PrecisionTargetFinder.init = function (self, is_server, is_local_unit, player, physics_world, unit)
	self._unit = unit
...
PrecisionTargetFinder.extensions_ready = function (self)
	local unit = self._unit

	self._weapon_extension = ScriptUnit.extension(unit, "weapon_system")
end

PrecisionTargetFinder.update_precision_target = function (self, unit, smart_targeting_template, ray_origin, forward, right, up, targeting_data, fixed_frame, visibility_cache, visibility_check_frame, optional_line_of_sight_cache)
	...
	local check_all_possible_validations = precision_target_settings.check_all_possible_validations
	...
	while num_hits >= i + 1 and not early_exit do
		repeat
			i = i + 1
			...
			if ignore_caster and hit_unit == unit then
				break
			end

			if check_all_possible_validations or smart_tagging then
			...
			if check_all_possible_validations or not smart_tagging then
				if unit_data_extension and HEALTH_ALIVE[hit_unit] then
					is_valid = true
			...
			if include_even_if_prop and is_valid and best_score ~= math.huge then
				...
				do
					...
					local half_width, half_height, target_weight
					...
					if min_range < distance and distance <= max_unit_range then
						...
						if breed then
							...
							visible_target, aim_position = true, hit_unit_center_pos
						end
						...
						if direct_hit and in_front_check then
							best_unit = hit_unit
...
PrecisionTargetFinder._target_visibility_and_aim_position = function (self, ray_origin, forward, right, up, hit_unit_center_pos, distance_to_center_pos, half_width, half_height, x_diff_no_abs, hit_unit, fixed_frame, visibility_cache, visibility_check_frame)
...
PrecisionTargetFinder.assisted_hitscan_trajectory = function (self, targeting_data, smart_targeting_template, weapon_template, raw_aim_rotation)
	...
	local wanted_rotation = Quaternion.look(targeting_data.target_position:unbox() - first_person_position)
...
implements(PrecisionTargetFinder, PrecisionTargetFinderInterface)

return PrecisionTargetFinder
