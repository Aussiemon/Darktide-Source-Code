...
MinionFxExtension.fixed_update = function (self, unit, dt, t, fixed_frame)
	return
end
...
MinionFxExtension.trigger_inventory_vfx = function (self, vfx_name, inventory_slot_name, fx_source_name, optional_lookup_fx_sources)
	self:_trigger_inventory_vfx(vfx_name, inventory_slot_name, fx_source_name, optional_lookup_fx_sources)
	...
	Managers.state.game_session:send_rpc_clients("rpc_trigger_minion_inventory_vfx", game_object_id, vfx_id, inventory_slot_id, fx_source_name_id, optional_lookup_fx_sources)
end

local ORPHANED_POLICY = "stop"

MinionFxExtension._trigger_inventory_vfx = function (self, vfx_name, inventory_slot_name, fx_source_name, optional_lookup_fx_sources)
	local visual_loadout_extension, world = self._visual_loadout_extension, self._world
	local inventory_item = visual_loadout_extension:slot_item(inventory_slot_name)
	local attachment_unit, node = MinionVisualLoadout.attachment_unit_and_node_from_node_name(inventory_item, fx_source_name, optional_lookup_fx_sources)
...
MinionFxExtension.rpc_trigger_minion_inventory_vfx = function (self, channel_id, go_id, vfx_id, inventory_slot_id, fx_source_name_id, optional_lookup_fx_sources)
...
MinionFxExtension._spawn_particles_on_line_with_interval = function (self, world, spawner_position, line_direction, line_rotation, line_length, vfx, interval)
	...
	while spawn_emitters do
		...
		if line_length < new_emitter_distance + 1 or num_emitters >= MAX_EMITTERS then
			...
			World.create_particles(self._world, vfx, spawn_pos, line_rotation)
...
MinionFxExtension._trigger_unit_line_fx = function (self, line_effect, inventory_slot_name, fx_source_name, end_position)
	...
	if effect_name then
		local particle_id = World.create_particles(world, effect_name, spawner_position, line_rotation)
		local variable_index = World.find_particles_variable(world, effect_name, "hit_distance")
...
MinionFxExtension.sound_source = function (self, source_name)
	local sources = self._sources

	return sources[source_name]
end

MinionFxExtension.register_sound_source = function (self, source_name, parent_unit, optional_node_name)
	local sources = self._sources
...
MinionFxExtension.unregister_sound_source = function (self, source_name)
	local sources = self._sources
	local wwise_world = self._wwise_world
