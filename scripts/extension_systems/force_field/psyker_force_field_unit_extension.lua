-- chunkname: @scripts/extension_systems/force_field/psyker_force_field_unit_extension.lua
...
local PsykerForceFieldUnitExtension = class("PsykerForceFieldUnitExtension")
...
PsykerForceFieldUnitExtension.init = function (self, extension_init_context, unit, extension_init_data, game_object_data_or_game_session, unit_spawn_parameter_or_game_object_id)
	self._unit = unit
	...
	self._particles_sphere = PARTICLES_SPHERE
	self._sound_events_sphere = SOUND_EVENTS_SPHERE

	local deployable_settings = extension_init_data.deployable_settings

	if deployable_settings then
		self._duration = deployable_settings.duration or self._duration
		self._particles_sphere = deployable_settings.particles_sphere or self._particles_sphere
		self._sound_events_sphere = deployable_settings.sound_events_sphere or self._sound_events_sphere
	end

	self._max_duration = self._duration

	local start_sound_event = sphere_shield and self._sound_events_sphere.start or SOUND_EVENTS_WALL.start
	...
	local start_particle_effect = sphere_shield and self._particles_sphere.start or PARTICLES_WALL.start
	...
	self._in_shield_buff_template_name = nil
	...
	if is_server then
		...
		if has_special_rule and sphere_shield then
			self._in_shield_buff_template_name = "psyker_boost_allies_in_sphere_buff"
			self._leaving_shield_buff_template_name = nil
...
PsykerForceFieldUnitExtension.destroy = function (self)
...
PsykerForceFieldUnitExtension.game_object_initialized = function (self, session, object_id)
...
PsykerForceFieldUnitExtension.extensions_ready = function (self, world, unit)
	self._health_extension = ScriptUnit.extension(unit, "health_system")
end

PsykerForceFieldUnitExtension.fixed_update = function (self, unit, dt, t)
...
PsykerForceFieldUnitExtension.update = function (self, unit, dt, t)
	self:_update_effects(dt, t)
end

PsykerForceFieldUnitExtension._update_effects = function (self, dt, t)
...
PsykerForceFieldUnitExtension.is_unit_colliding = function (self, unit_pos, unit_radius, handle_height)
...
PsykerForceFieldUnitExtension.force_field_unit = function (self)
	return self._unit
end

PsykerForceFieldUnitExtension.remaining_duration = function (self)
	return self._duration
end

PsykerForceFieldUnitExtension.remaining_life = function (self)
...
PsykerForceFieldUnitExtension.is_dead = function (self)
	return self._health_extension:is_dead()
end

PsykerForceFieldUnitExtension.is_sphere_shield = function (self)
	return self._sphere_shield
end

PsykerForceFieldUnitExtension.reflected_direction = function (self, unit, direction)
	local normal
...
PsykerForceFieldUnitExtension.on_player_enter = function (self, unit, t)
	...
	local in_shield_buff_template_name = self._in_shield_buff_template_name

	if in_shield_buff_template_name then
		local _, local_index, component_index = unit_buff_extension:add_externally_controlled_buff(in_shield_buff_template_name, t, "owner_unit", self._unit)
...
PsykerForceFieldUnitExtension.on_player_exit = function (self, unit, t)
	if not self._is_server then
		return
	end
...
PsykerForceFieldUnitExtension.on_death = function (self, t)
...
PsykerForceFieldUnitExtension._trigger_death_effects = function (self)
	local world = self._world
	local wwise_world = self._wwise_world
	local source_id = self._source_id
	local effect_id = self._effect_id
	local sphere_shield = self._sphere_shield
	local stop_sound_event = sphere_shield and self._sound_events_sphere.stop or SOUND_EVENTS_WALL.stop
	...
	local stop_particle_effect = sphere_shield and self._particles_sphere.stop or PARTICLES_WALL.stop

	if effect_id and World.are_particles_playing(world, effect_id) then
		World.destroy_particles(world, effect_id)
	end

	local stop_flow_event = sphere_shield and self._particles_sphere.stop_flow_event or nil

	if stop_flow_event then
		Unit.flow_event(self._unit, stop_flow_event)
	end

	self._effect_id = World.create_particles(world, stop_particle_effect, self._position:unbox(), self._rotation:unbox())
end

implements(PsykerForceFieldUnitExtension, ForceFieldExtensionInterface)

return PsykerForceFieldUnitExtension
