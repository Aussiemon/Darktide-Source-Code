...
LuggableSynchronizerExtension.init = function (self, extension_init_context, unit, extension_init_data, ...)
	...
	self._luggable_despawn_timer = 0
	self._spawner_respawn_timers = {}
	self._luggable_despawn_timers = {}
	self._spawners_with_marker_on_start = {}
	self._objective_stages = 1
	self._is_side_mission_synchronizer = false

	fassert(Unit.has_volume(unit, "g_luggable_safe_zone"), "[LuggableSynchronizerExtension][init][Unit: %s] Missing volume 'g_luggable_safe_zone'.", Unit.id_string(unit))
end

LuggableSynchronizerExtension.setup_from_component = function (self, objective_name, objective_stages, auto_start, manual_luggable_spawn, max_socket_target, keep_unused_sockets, luggable_should_respawn, luggable_respawn_timer, luggable_despawn_timer, is_side_mission_synchronizer, automatic_start_on_level_spawned)
	...
	self._luggable_despawn_timer = luggable_despawn_timer
...
LuggableSynchronizerExtension.fixed_update = function (self, unit, dt, t)
	if self._is_server and self._mission_active then
		...
		local despawn_num = self:_check_luggable_to_despawn(dt)
		local has_despawned = despawn_num > 0
...
LuggableSynchronizerExtension._check_luggable_to_despawn = function (self, dt)
	fassert(self._is_server, "[LuggableSynchronizerExtension] Server only method.")

	local luggable_despawn_timers = self._luggable_despawn_timers
	local despawn_num = 0

	for luggable_unit, timer in pairs(luggable_despawn_timers) do
		timer = timer - dt
		timer = math.max(0, timer)
		...
		if timer == 0 then
			...
			self._luggable_despawn_timers[luggable_unit] = nil
			self._spawner_to_luggable[luggable_unit] = nil
			despawn_num = despawn_num + 1
		else
			self._luggable_despawn_timers[luggable_unit] = timer
		end
	end
			...
			for spawner_unit, luggable_unit in pairs(spawner_to_luggable) do
				...
				if has_overlap then
					...
					if not consume_luggable then
						...
						self._luggable_despawn_timers[luggable_unit] = self._luggable_despawn_timer
					end
				end
			end
		end
...
LuggableSynchronizerExtension.spawn_luggable = function (self, spawner_unit)
	...
	if self._mission_active then
		fassert(self._spawner_to_luggable[spawner_unit] == nil, "[LuggableSynchronizerExtension][_spawn_luggable][%s, %s] Spawner already used.", Unit.id_string(spawner_unit), spawner_unit)
		...
		fassert(luggable_unit, "Could not spawn luggable.")
...
LuggableSynchronizerExtension._despawn_luggable = function (self, luggable_unit_to_del, respawn_at_spawner)
	...
	fassert(spawner_unit_to_use, "[LuggableSynchronizerExtension][_despawn_luggable] could not find spawner.")
