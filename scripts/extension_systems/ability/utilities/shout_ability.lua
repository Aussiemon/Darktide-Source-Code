-- chunkname: @scripts/extension_systems/ability/utilities/shout_ability.lua
...
function _handle_enemy_targets(t, radius, target_settings, player_unit, locomotion_component, player_side, player_buff_extension, backup_position, backup_rotation, external_power_modifier)
	...
	for ii = 1, num_hits do
		repeat
			...
			local disallowed_breeds = target_settings.ignored_breeds

			if disallowed_breeds then
				...
				if disallowed_breeds[breed_name] then
					can_hit = false
				end
			end
