...
PickupSystem._spawn_open_pickups = function (self, distribution_type, pickup_pool, usable_spawners, pickups_to_spawn, seed)
	table.clear(pickups_to_spawn)
...
PickupSystem._check_spawn = function (self, spawner, pickups_to_spawn, pickup_type)
	local num_pickups_to_spawn = #pickups_to_spawn
	local spawner_extension = spawner.extension
...
PickupSystem._skip_group_blocks = function (self, skip_group)
...
PickupSystem.rpc_move_pickup = function (self, channel_id, pickup_id, pickup_is_level_unit, position, rotation)
	local unit_spawner_manager = Managers.state.unit_spawner
	local pickup_unit = unit_spawner_manager:unit(pickup_id, pickup_is_level_unit)

	self:move_pickup(pickup_unit, position, rotation)
end

PickupSystem.move_pickup = function (self, unit, position, rotation)
	...
	if self._is_server then
		local unit_spawner_manager = Managers.state.unit_spawner
		local pickup_is_level_unit, pickup_id = unit_spawner_manager:game_object_id_or_level_index(unit)

		Managers.state.game_session:send_rpc_clients("rpc_move_pickup", pickup_id, pickup_is_level_unit, position, rotation)
	end
end
