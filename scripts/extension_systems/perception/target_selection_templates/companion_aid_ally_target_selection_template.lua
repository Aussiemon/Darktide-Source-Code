-- chunkname: @scripts/extension_systems/perception/target_selection_templates/companion_aid_ally_target_selection_template.lua
...
local function _reference_position(unit, side)
	local vector3_distance_squared = Vector3.distance_squared
	local own_pos = POSITION_LOOKUP[unit]
	local reference_position, best_dist_sq = nil, math.huge
	local ally_positions = side.valid_human_units_positions

	for i = 1, #ally_positions do
		local ally_pos = ally_positions[i]
		local dist_sq = vector3_distance_squared(own_pos, ally_pos)

		if dist_sq < best_dist_sq then
			best_dist_sq = dist_sq
			reference_position = ally_pos
		end
	end

	if not reference_position then
		...
		if not reference_position then
			local ai_units = side:relation_units("allied")

			for i = 1, #ai_units do
				local ally_pos = POSITION_LOOKUP[ai_units[i]]
				local dist_sq = ally_pos and vector3_distance_squared(own_pos, ally_pos) or math.huge
	...
	return reference_position or own_pos
end
...
target_selection_template.aid_ally = function (unit, side, perception_component, buff_extension, breed, target_units, line_of_sight_lookup, t, threat_units, force_new_target_attempt, force_new_target_attempt_config_or_nil, debug_target_weighting_or_nil)
	...
	local vector3_distance_squared = Vector3.distance_squared
	local reference_position = _reference_position(unit, side)
	...
	if target_units[current_target_unit] then
		...
		local distance_sq = vector3_distance_squared(reference_position, target_position)
		local z_distance = math.abs(reference_position.z - target_position.z)
	...
	if not lock_target then
		...
		if target_units[taunter_unit] then
			...
			local distance_to_target_sq = vector3_distance_squared(reference_position, target_position)
			local z_distance = math.abs(reference_position.z - target_position.z)
