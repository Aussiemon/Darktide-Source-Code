-- chunkname: @scripts/extension_systems/perception/bot_perception_extension.lua
...
BotPerceptionExtension.pre_update = function (self, unit, dt, t)
	...
	local num_hits = broadphase.query(broadphase, search_position, FORCED_PRIO_UPDATE_RANGE, broadphase_results, enemy_side_names)
...
BotPerceptionExtension.update = function (self, unit, dt, t)
	...
	local side_system, perception_component = self._side_system, self._perception_component
	local behavior_component = self._behavior_component
	local bot_group = self._bot_group
	local side, position = side_system.side_by_unit[unit], POSITION_LOOKUP[unit]
...
BotPerceptionExtension._update_target_enemy = function (self, self_unit, self_position, perception_component, behavior_component, enemies_in_proximity, side, bot_group, dt, t)
	...
	if t > self._enemies_in_proximity_update_timer then
		...
		for read_index = 1, num_enemies_in_proximity do
			...
			if not breed.not_bot_target and aggroed_units[enemy_unit] then
				...
				if write_index >= MAX_PROXIMITY_ENEMIES then
					break
				end
			end
		end
...
BotPerceptionExtension.within_aid_range = function (self, unit, perception_component)
	if perception_component.target_ally_needs_aid then
		...
		local self_position, target_position = POSITION_LOOKUP[unit], POSITION_LOOKUP[target_ally]
...
BotPerceptionExtension._update_target_ally = function (self, self_unit, self_position, perception_component, side, bot_group, t)
	local best_ally, ally_distance, in_need_type, look_at_ally
	...
	if best_ally and in_need_type then
		perception_component.target_ally_needs_aid, perception_component.target_ally_need_type = true, in_need_type
	elseif perception_component.target_ally_needs_aid then
		perception_component.target_ally_needs_aid, perception_component.target_ally_need_type = false, "n/a"
	end
...
BotPerceptionExtension._select_ally_by_utility = function (self, self_unit, self_position, perception_component, side, t)
	local can_heal_other, can_give_healing_to_other, self_health_utility = false, false, 0
	local main_path_manager = Managers.state.main_path
	local player_unit_spawn_manager = Managers.state.player_unit_spawn
	local closest_ally, closest_in_need_type, closest_ally_look_at = nil, nil, false
	local closest_distance, closest_real_distance = math.huge, math.huge
				...
				if allowed_follow_path then
					...
					elseif in_need_type then
						...
						for j = 1, num_alive_monsters do
							...
							if monster_target == self_unit and self_to_monster_distance_sq < ALLOWED_AID_MIN_RANGE_MONSTER_TARGETING_SELF_SQ then
								in_need_type, utility = nil, 0

								break
							end

							if player_health > MIN_ALLY_HEALTH_TO_CONSIDER_MONSTERS then
					...
					if in_need_type or not is_bot then
						...
						if distance < closest_distance then
							closest_distance, closest_real_distance = distance, real_distance
							closest_ally, closest_in_need_type, closest_ally_look_at = player_unit, in_need_type, look_at_ally
						end
					end
				end
			end
...
BotPerceptionExtension._select_next_health_station = function (self, self_unit, self_position, perception_component, t)
	local unit, distance = nil, math.huge
...
local IS_WOUNDED_MODIFIER, IS_WOUNDED_QUICK_USE_MODIFIER = 0.33, 0.5
...
local WANTS_TO_HEAL_THRESHOLD, WANTS_TO_GIVE_HEAL_TO_OTHER = 0.25, 0.5
local KNOCKED_DOWN_UTILITY, LEDGE_HANGING_UTILITY, NETTED_UTILITY, HOGTIED_UTILITY = 100, 100, 100, 100
local HEAL_OTHER_BASE_UTILITY, HEAL_OTHER_HEALTH_UTILITY_MODIFIER = 70, 15
local GIVE_HEAL_TO_OTHER_BASE_UTILITY, GIVE_HEAL_TO_OTHER_HEALTH_UTILITY_MODIFER = 70, 10
local STOP_BASE_UTILITY, LOOK_AT_BASE_UTILITY = 5, 2
...
BotPerceptionExtension._calculate_ally_need_type = function (self, self_position, self_health_utility, can_heal_other, can_give_healing_to_other, ally_unit, ally_position, target_enemy, ally_is_bot, t, priority_target_enemy)
	...
	local in_need_type, look_at_ally, utility = nil, false, 0

	if ally_is_bot then
		if self._num_enemies_in_proximity >= MAX_ENEMIES_IN_PROXIMITY_TO_AID_BOT then
	...
	elseif PlayerUnitStatus.is_hogtied(ally_character_state_component) then
		...
		if can_heal_other and (health_percent < WANTS_TO_HEAL_THRESHOLD or is_wounded) and heal_other_allowed then
			in_need_type, utility = "in_need_of_heal", HEAL_OTHER_BASE_UTILITY + health_utility * HEAL_OTHER_HEALTH_UTILITY_MODIFIER
		elseif can_give_healing_to_other and is_wounded and (health_percent < WANTS_TO_GIVE_HEAL_TO_OTHER or is_wounded) and not has_healthkit and heal_other_allowed then
			in_need_type, utility = "can_accept_heal_item", GIVE_HEAL_TO_OTHER_BASE_UTILITY + health_utility * GIVE_HEAL_TO_OTHER_HEALTH_UTILITY_MODIFER
		elseif need_attention_type == "stop" then
			in_need_type, look_at_ally, utility = "in_need_of_attention_stop", true, STOP_BASE_UTILITY + extra_utility
		elseif need_attention_type == "look_at" then
			in_need_type, look_at_ally, utility = "in_need_of_attention_look", true, LOOK_AT_BASE_UTILITY + extra_utility
		end
	end
...
local ATTENTION_MIN_STOP_DISTANCE, ATTENTION_MAX_STOP_DISTANCE = 0.5, 3.5
...
local STOP_EXTRA_UTILITY_MIN, STOP_EXTRA_UTILITY_MAX = 0, 2
local LOOK_AT_EXTRA_UTILITY_MIN, LOOK_AT_EXTRA_UTILITY_MAX = 0, 0.5
...
BotPerceptionExtension._player_needs_attention = function (self, self_position, player_unit, player_position, player_unit_data_extension, target_enemy, t)
	...
	if speed_sq > SPEED_EPSILON_SQ then
	...
	if smallest_speed_sq > SPEED_EPSILON_SQ or is_heading_towards_player then
		stop_threshold, look_threshold = BOTH_MOVING_WITHOUT_ITEM_TO_GIVE_STOP_THRESHOLD, BOTH_MOVING_WITHOUT_ITEM_TO_GIVE_LOOK_AT_THRESHOLD
	elseif speed_sq <= 0.01 and player_speed_sq <= 0.01 then
		stop_threshold, look_threshold = STOPPED_WITHOUT_ITEM_TO_GIVE_STOP_THRESHOLD, STOPPED_WITHOUT_ITEM_TO_GIVE_LOOK_AT_THRESHOLD
	else
		stop_threshold, look_threshold = MOVING_WITHOUT_ITEM_TO_GIVE_STOP_THRESHOLD, MOVING_WITHOUT_ITEM_TO_GIVE_LOOK_AT_THRESHOLD
	end
...
local ALLY_PATH_TIME_MIN_DISTANCE, ALLY_PATH_TIME_MAX_DISTANCE = 5, 15
local ALLY_PATH_TIME_DISTANCE_SLOPE = 1 / (ALLY_PATH_TIME_MAX_DISTANCE - ALLY_PATH_TIME_MIN_DISTANCE)
local ALLY_PATH_MIN_DISTANCE_TIME, ALLY_PATH_MAX_DISTANCE_TIME = 3, 12
...
BotPerceptionExtension._ally_path_allowed = function (self, self_position, self_segment_index, ally_unit, ally_position, ally_segment_index, t)
	...
	if path_status and path_status.failed then
		...
		ignore_for = self_segment_index < ally_segment_index and 1 or ally_segment_index < self_segment_index and 10 or 5
		...
		if no_longer_ignored then
			...
			local has_moved = Vector3.distance_squared(ally_position, last_path_destination) > ALLY_PATH_FAILED_REPATH_THRESHOLD_SQ
