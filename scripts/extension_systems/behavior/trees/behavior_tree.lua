...
require("scripts/extension_systems/behavior/nodes/actions/bt_beast_of_nurgle_follow_action")
...
require("scripts/extension_systems/behavior/nodes/actions/bt_shoot_flamer_action")
...
local function _create_btnode_from_lua_node(lua_node, parent_btnode)
	...
	fassert(identifier, "[BehaviorTree] Tree node with parent %q is missing name.", lua_node.name)
	...
	fassert(action_data == nil or identifier == action_data.name, "[BehaviorTree] Mismatching node and action name (node=%q, action=%q).", identifier, action_data and action_data.name)
