-- chunkname: @scripts/extension_systems/behavior/nodes/generated/bt_cultist_berzerker_selector_node.lua

require("scripts/extension_systems/behavior/nodes/bt_node")

local BtCultistBerzerkerSelectorNode = class("BtCultistBerzerkerSelectorNode", "BtNode")

BtCultistBerzerkerSelectorNode.init = function (self, ...)
	BtCultistBerzerkerSelectorNode.super.init(self, ...)

	self._children = {}
end

BtCultistBerzerkerSelectorNode.init_values = function (self, blackboard, action_data, node_data)
...
BtCultistBerzerkerSelectorNode.add_child = function (self, node)
	self._children[#self._children + 1] = node
end

BtCultistBerzerkerSelectorNode.evaluate = function (self, unit, blackboard, scratchpad, dt, t, evaluate_utility, node_data, old_running_child_nodes, new_running_child_nodes, last_leaf_node_running)
	...
	do
		local node_close_melee_combat = children[9]
		local tree_node = node_close_melee_combat.tree_node
		local condition_args = tree_node.condition_args
		local is_running = last_leaf_node_running and last_running_node == node_close_melee_combat
		local condition_result
		...
		if condition_result then
			local leaf_node = node_close_melee_combat:evaluate(unit, blackboard, scratchpad, dt, t, evaluate_utility, node_data, old_running_child_nodes, new_running_child_nodes, last_leaf_node_running)

			if leaf_node then
				new_running_child_nodes[node_identifier] = node_close_melee_combat
...
BtCultistBerzerkerSelectorNode.run = function (self, unit, breed, blackboard, scratchpad, action_data, dt, t, node_data, running_child_nodes)
	local node_identifier = self.identifier
