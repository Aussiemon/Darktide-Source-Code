-- chunkname: @scripts/extension_systems/behavior/nodes/actions/bt_chaos_hound_leap_action.lua
...
BtChaosHoundLeapAction.run = function (self, unit, breed, blackboard, scratchpad, action_data, dt, t)
	...
	if ALIVE[target_unit] and not scratchpad.extra_push_applied then
		...
		local unit_position, target_unit_position = POSITION_LOOKUP[unit], POSITION_LOOKUP[target_unit]

		if target_is_pushing and Vector3.distance_squared(unit_position, target_unit_position) < 25 then
			local damage_profile, power_level = action_data.push_enemies_damage_profile, action_data.push_enemies_power_level
...
BtChaosHoundLeapAction._update_starting_state = function (self, unit, scratchpad, action_data, dt, t, locomotion_extension, perception_component, target_unit)
	...
	local leap_speed, leap_relax_distance = ChaosHoundSettings.leap_speed, ChaosHoundSettings.collision_radius
...
BtChaosHoundLeapAction._calculate_wanted_velocity = function (self, physics_world, start_position, target_position, target_velocity, speed, optional_relax_distance, debug)
	local gravity, acceptable_accuracy = ChaosHoundSettings.leap_gravity, ChaosHoundSettings.leap_acceptable_accuracy
	...
	local num_sections, collision_filter, radius = ChaosHoundSettings.leap_num_sections, ChaosHoundSettings.leap_collision_filter, ChaosHoundSettings.leap_radius
...
BtChaosHoundLeapAction._update_leaping_state = function (self, unit, scratchpad, action_data, dt, t, breed, locomotion_extension, target_unit)
	...
	elseif current_colliding_target then
		if ALIVE[current_colliding_target] then
			...
			local position, colliding_target_position = POSITION_LOOKUP[unit], POSITION_LOOKUP[current_colliding_target]
			...
			if t >= scratchpad.current_colliding_target_check_time then
				...
				if can_hit then
					...
					scratchpad.current_colliding_target, scratchpad.current_colliding_target_check_time = nil
				end
			end
	...
	elseif ALIVE[target_unit] then
		...
		local position, target_unit_position = POSITION_LOOKUP[unit], POSITION_LOOKUP[target_unit]
...
BtChaosHoundLeapAction._update_in_air_stagger = function (self, scratchpad, action_data, t)
	...
	if t > scratchpad.stagger_duration then
...
BtChaosHoundLeapAction._check_leap_for_collisions = function (self, scratchpad, action_data, physics_world, start_t, end_t, leap_start_position, leap_velocity)
	...
	local debug
	local collision_filter, gravity, radius = ChaosHoundSettings.leap_collision_filter, ChaosHoundSettings.leap_gravity, ChaosHoundSettings.leap_radius
...
local EPSILON = 0.01
local WALL_JUMP_NAV_ABOVE, WALL_JUMP_NAV_BELOW, WALL_JUMP_NAV_LATERAL = 1, 5, 1
...
BtChaosHoundLeapAction._calculate_wall_jump_velocity = function (self, action_data, physics_world, nav_world, traverse_logic, position, wall_normal)
	...
	local forward, forward_length = Quaternion.forward(rotation), ChaosHoundSettings.leap_radius + EPSILON
...
BtChaosHoundLeapAction._update_falling_state = function (self, unit, scratchpad, action_data, dt, t, locomotion_extension, target_unit)
	local current_velocity, gravity = locomotion_extension:current_velocity(), ChaosHoundSettings.leap_gravity
	local fall_speed = current_velocity.z - gravity * dt
	local current_position, landing_position = POSITION_LOOKUP[unit]

	if fall_speed < 0 then
		local above, below, nav_world, traverse_logic = 0.1, -fall_speed * dt, scratchpad.nav_world, scratchpad.traverse_logic
