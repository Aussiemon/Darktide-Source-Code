-- chunkname: @scripts/extension_systems/behavior/nodes/actions/bot/bt_bot_shoot_action.lua
...
BtBotShootAction.enter = function (self, unit, breed, blackboard, scratchpad, action_data, t)
	...
	local visual_loadout_extension, inventory_component = ScriptUnit.extension(unit, "visual_loadout_system"), unit_data_extension:read_component("inventory")
	...
	local perception_component, spawn_component, behavior_component = blackboard.perception, blackboard.spawn, blackboard.behavior
...
BtBotShootAction._should_aim = function (self, t, scratchpad, action_data)
	...
	local target_ally_unit, target_ally_needs_aid = perception_component.target_ally, perception_component.target_ally_needs_aid
	...
	if target_ally_distance > MIN_TARGET_ALLY_DISTANCE_FOR_AIM then
		return false
	end
...
BtBotShootAction._start_aiming = function (self, t, scratchpad)
	if not scratchpad.aiming_shot then
		...
		local aim_action_input, raw_input = scratchpad.aim_action_input
...
BtBotShootAction._stop_aiming = function (self, scratchpad)
	if scratchpad.aiming_shot then
		...
		local action_input_extension = scratchpad.action_input_extension
		local unaim_action_input, raw_input = scratchpad.unaim_action_input
...
BtBotShootAction._update_aim = function (self, unit, scratchpad, action_data, dt, t)
	...
	if t >= scratchpad.reevaluate_obstruction_time then
	...
	local evaluate = scratchpad.fired and t > scratchpad.next_evaluate or t > scratchpad.next_evaluate_without_firing
...
local PI, TWO_PI = math.pi, math.two_pi
...
BtBotShootAction._aim_position = function (self, self_unit, scratchpad, action_data, dt, current_position, current_rotation, target_unit)
	local projectile_template, aim_at_node
	...
	local new_yaw, new_pitch = current_yaw + yaw_speed * dt, current_pitch + pitch_speed * dt
	local actual_rotation = Quaternion.from_yaw_pitch_roll(new_yaw, new_pitch, 0)
	local yaw_offset, pitch_offset = (new_yaw - wanted_yaw + PI) % TWO_PI - PI, new_pitch - wanted_pitch
...
BtBotShootAction._wanted_aim_rotation = function (self, self_unit, target_unit, target_breed, current_position, projectile_template, aim_at_node)
	...
	if projectile_template and projectile_template.gravity then
		...
		local projectile_speed, projectile_gravity = projectile_template.speed, projectile_template.gravity
...
local YAW_ACCELERATION, YAW_DECELERATION = 7.5, 35
...
BtBotShootAction._calculate_aim_speed = function (self, dt, current_yaw, current_pitch, wanted_yaw, wanted_pitch, current_yaw_speed, action_data)
	...
	local new_yaw_speed
	local yaw_offset_sign, yaw_speed_sign = math.sign(yaw_offset), math.sign(current_yaw_speed)
...
BtBotShootAction._reevaluate_obstruction = function (self, self_unit, scratchpad, action_data, t, ray_from, wanted_aim_rotation, actual_aim_position, target_unit)
	...
	local min, max = action_data.minimum_obstruction_reevaluation_time, action_data.maximum_obstruction_reevaluation_time
...
BtBotShootAction._update_collision_filter = function (self, target_unit, scratchpad)
	local perception_component = scratchpad.perception_component
	local target_ally_unit, target_ally_need_type = perception_component.target_ally, perception_component.target_ally_need_type
	...
	if ff_ranged then
		ignore_hitting_allies, ignore_hitting_allies_charged = attack_meta_data.ignore_allies_for_obstruction, attack_meta_data.ignore_allies_for_obstruction_charged
	else
		ignore_hitting_allies, ignore_hitting_allies_charged = true, true
	end

	scratchpad.ignore_hitting_allies, scratchpad.ignore_hitting_allies_charged = ignore_hitting_allies, ignore_hitting_allies_charged
	scratchpad.ignore_hitting_enemies, scratchpad.ignore_hitting_enemies_charged = ignore_enemies_for_obstruction, ignore_enemies_for_obstruction_charged
...
BtBotShootAction._is_shot_obstructed = function (self, self_unit, scratchpad, collision_filter, from, direction, target_unit, actual_aim_position, ignore_allies, ignore_enemies)
	local physics_world, max_distance = scratchpad.physics_world, Vector3.distance(from, actual_aim_position)
	...
	local allied_units_lookup, enemy_units_lookup = side.allied_units_lookup, side.enemy_units_lookup
	local Actor_unit, HitZone_get_name, Health_is_ragdolled = Actor.unit, HitZone.get_name, Health.is_ragdolled
	...
	for i = 1, num_hits do
		repeat
			...
			if hit_zone_name_or_nil == AFRO_HITZONE or allied_units_lookup[hit_unit] and ignore_allies or enemy_units_lookup[hit_unit] and ignore_enemies then
				do break end
				break
			end
			...
			if hit_unit ~= self_unit and not Health_is_ragdolled(hit_unit) then
...
BtBotShootAction._should_charge = function (self, scratchpad, range_squared, target_unit, t)
	...
	local weapon_extension = scratchpad.weapon_extension
	local charge_action_input, used_input = scratchpad.charge_action_input
	local fixed_t = FixedFrame.get_latest_fixed_time()
	...
	if scratchpad.always_charge_before_firing or scratchpad.charge_range_sq and range_squared >= scratchpad.charge_range_sq then
		return true
	end
...
BtBotShootAction._charge_shot = function (self, scratchpad, action_data, bot_unit_input, t)
	if not scratchpad.charging_shot then
		scratchpad.charging_shot, scratchpad.charge_start_time = true, t

		local action_input_extension = scratchpad.action_input_extension
		local charge_action_input, raw_input = scratchpad.charge_action_input
...
BtBotShootAction._aim_good_enough = function (self, unit, dt, t, scratchpad, yaw_offset, pitch_offset)
	if t > scratchpad.reevaluate_aim_time then
		...
		if offset > aim_data.max_radius then
			...
			if success then
				scratchpad.aim_good_enough, scratchpad.num_aim_rolls = true, 0
			else
				scratchpad.aim_good_enough, scratchpad.num_aim_rolls = false, num_rolls
			end
		end
...
BtBotShootAction._may_fire = function (self, unit, scratchpad, range_squared, t)
	...
	local charging, minimum_charge_time = scratchpad.charging_shot, scratchpad.minimum_charge_time
	...
	local weapon_extension = scratchpad.weapon_extension
	local fire_action_input, used_input = scratchpad.fire_action_input
	local fixed_t = FixedFrame.get_latest_fixed_time()
...
BtBotShootAction._fire = function (self, scratchpad, action_data, bot_unit_input, t)
	...
	local fire_action_input, aim_fire_action_input = scratchpad.fire_action_input, scratchpad.aim_fire_action_input
	local action_input, raw_input = aiming_shot and aim_fire_action_input or fire_action_input
