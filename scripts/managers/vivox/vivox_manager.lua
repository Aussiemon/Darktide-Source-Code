...
VivoxManager.leave_channel = function (self, channel_handle)
	if self:is_logged_in() then
		Log.info("VivoxManager", "Leaving channel %s", channel_handle)
		Vivox.remove_session(channel_handle)
	end
end

VivoxManager.sessions = function (self)
...
VivoxManager.send_loc_channel_message = function (self, channel_handle, key, context)
	local status, result = pcall(cjson.encode, context or {})

	if not status then
		Log.error("VivoxManager", "Could not encode context: " .. result)
...
VivoxManager.channel_text_mute_participant = function (self, channel_handle, participant_uri, mute)
	Log.info("VivoxManager", "Text %s participant %s in channel %s", mute and "muting" or "unmuting", participant_uri, channel_handle)
...
VivoxManager.channel_voip_mute_participant = function (self, channel_handle, participant_uri, mute)
	Log.info("VivoxManager", "Voip %s participant %s in channel %s", mute and "muting" or "unmuting", participant_uri, channel_handle)
	Vivox.audio_session_set_participant_mute_for_me(channel_handle, participant_uri, mute)
end

VivoxManager.player_mute_status_changed = function (self, account_id)
	for session_handle, channel in pairs(self._sessions) do
		...
		if party_id and Managers.party_immaterium:num_other_members() > 0 then
			if not self._party_id_session_handles[party_id] then
				Managers.party_immaterium:request_vivox_party_token():next(function (vivox_data)
					...
					Log.error("VivoxManager", "Error connecting to party channel: " .. tostring(error))
				end)
			end
		else
		...
		if participant.is_validated == true then
			if string.sub(message.message_body, 1, 5) == "<loc|" then
				...
				if #parts == 3 then
					...
					if not status then
						Log.error("VivoxManager", "Could not decode context: " .. parts[3])
...
VivoxManager._update_transmitting_channel_priority = function (self)
	local priority = {
