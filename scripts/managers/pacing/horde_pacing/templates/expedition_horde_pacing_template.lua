-- chunkname: @scripts/managers/pacing/horde_pacing/templates/expedition_horde_pacing_template.lua
...
local STINGER = "wwise/events/minions/play_minion_expeditions_horde_signal_3d_med"
local PRE_STINGER = "wwise/events/minions/play_minion_expeditions_horde_signal_2d"
...
local DEFAULT_TRICKLE_HORDE_COMPOSITIONS = {
	renegade = {
		...
		alert = {
			...
			HordeCompositions.expedition_renegade_trickle_ranged_alert,
	...
	cultist = {
		...
		alert = {
			...
			HordeCompositions.expedition_cultist_trickle_ranged_alert,
...
local function high_heat()
	local current_stage = Managers.state.pacing:current_stage_name()
	local has_current_stage = current_stage == "detected" or current_stage == "max"

	if has_current_stage then
		return true
	end
...
local function medium_heat()
	local current_stage = Managers.state.pacing:current_stage_name()
	local has_current_stage = current_stage == "alert" or current_stage == "detected" or current_stage == "max"
...
local horde_pacing_template = {
	name = "expedition_horde",
	resistance_templates = {
		{
			aggro_nearby_roamers_zone_range = 1,
			horde_type_for_limited_ambush_hordes = "far_distance_horde",
					...
					horde_setup = {
						{
							...
							random_targets = true,
							skip_spawners = true,
							stinger = "wwise/events/minions/play_minion_captain_spawn_stinger",
