-- chunkname: @scripts/managers/imgui/widgets/imgui_widget_utilities.lua

local ImguiWidgetUtilities = {}

ImguiWidgetUtilities.focus_widget = function (label)
...
ImguiWidgetUtilities.activate_widget = function (label)
	local id = Imgui.get_id(label)

	Imgui.activate_item(id)
end

ImguiWidgetUtilities.open_tree_node = function (label)
	local id = Imgui.get_id(label)

	Imgui.set_state_storage_integer(id, 1)
end

ImguiWidgetUtilities.close_tree_node = function (label)
	local id = Imgui.get_id(label)

	Imgui.set_state_storage_integer(id, 0)
end

ImguiWidgetUtilities.is_tree_node_open = function (label)
	local id = Imgui.get_id(label)
...
ImguiWidgetUtilities.scroll_last_item_into_view = function ()
...
ImguiWidgetUtilities.filter_functions = {
	...
	match_substring_replace_underscore = function (source_table, result_table, filter_string)
		...
		for i = 1, #source_table do
			...
			if not TEMP_FOUND_ENTRIES[value] and string.find(string.lower(value), filter_string, initial_index, use_plain_search) then
				result_table[result_index] = value
				result_index = result_index + 1
			end
		end
...
ImguiWidgetUtilities.filter_array_defaults = function (source_table, result_table, current_values, defaults)
	...
	for i = 1, #source_table do
		local widget_name = source_table[i]

		if filter[widget_name] then
			result_table[#result_table + 1] = source_table[i]
		end
	end
end
