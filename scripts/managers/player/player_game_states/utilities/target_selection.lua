...
local TargetSelection = {
	select_target = function (physics_world, self_unit, ray_origin, ray_rotation, collision_filter)
		local selected_unit, selected_unit_distance = nil
			...
			if actor then
				...
				if hit_unit ~= self_unit then
					...
					if is_pickup or is_alive then
						local half_width, half_height, hit_unit_center_pos = nil
						...
						if MINIMUM_DISTANCE < distance then
