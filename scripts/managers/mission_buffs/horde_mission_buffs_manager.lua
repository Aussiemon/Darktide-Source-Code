-- chunkname: @scripts/managers/mission_buffs/horde_mission_buffs_manager.lua

require("scripts/managers/mission_buffs/mission_buffs_manager")
...
local HordeMissionBuffsManager = class("HordeMissionBuffsManager", "MissionBuffsManager")
local CLIENT_RPCS = {
...
HordeMissionBuffsManager._unregister_rpcs = function (self, network_event_delegate)
	if not self._is_server then
		network_event_delegate:unregister_events(unpack(CLIENT_RPCS))
	end
end

HordeMissionBuffsManager._register_events = function (self)
	for _, event in ipairs(EVENTS) do
		Managers.event:register(self, event[1], event[2])
	end
end

HordeMissionBuffsManager._unregister_events = function (self)
	for _, event in ipairs(EVENTS) do
		Managers.event:unregister(self, event[1])
	end
end

HordeMissionBuffsManager.init = function (self, is_server, game_mode, game_mode_name, network_event_delegate)
...
HordeMissionBuffsManager.destroy = function (self)
...
HordeMissionBuffsManager._fetch_backend_data_needed_before_player_data_initialization = function (self)
...
HordeMissionBuffsManager.cb_get_horde_setting_from_the_backend_received = function (self, horde_backend_data)
...
HordeMissionBuffsManager.cb_get_horde_setting_from_the_backend_failed = function (self, err)
	Log.error("HordeMissionBuffsManager", string.format("Could not get backend list of settings.\nNo buffs will be excluded, Buff Family and Islands will default to even weights.\nError: %s", err.description))
...
HordeMissionBuffsManager._init_randomization_settings = function (self, horde_backend_data)
...
HordeMissionBuffsManager._init_backend_deactivated_buffs = function (self, horde_backend_data)
	...
	if deactivated_buffs_data.deactivated_buffs_enabled and not deactivated_buffs_data.deactivated_buffs_list then
		Log.error("HordeMissionBuffsManager", "Backend list of hordes settings does not have list of deactivated buffs but has it enabled (none will be excluded).")
	end
	...
	Log.info("HordeMissionBuffsManager", excluded_buffs_message)
end

HordeMissionBuffsManager.check_if_all_players_chosen_family = function (self)
	self._mission_buffs_handler:check_if_all_players_chosen_family()
end

HordeMissionBuffsManager.does_player_have_buff_saved = function (self, player, buff_name)
	return self._mission_buffs_handler:does_player_have_buff_saved(player, buff_name)
end

HordeMissionBuffsManager.try_show_new_ui_notification = function (self)
	self._mission_buffs_ui_manager:try_show_new_ui_notification()
end

HordeMissionBuffsManager.get_buffs_to_exclude = function (self)
	return self._backend_buffs_to_exclude
end

HordeMissionBuffsManager.get_weighted_randomization_data = function (self)
	return self._backend_weighted_randomization
end

HordeMissionBuffsManager.get_buff_family_selected_by_player = function (self, player)
...
HordeMissionBuffsManager.check_catchup_for_new_player = function (self, player, override_waves_completed_num)
	...
	if not does_player_have_family_selected then
		Log.error("HordeMissionBuffsManager", "Tried triggering catchup on player without family selected! Once they select their family, catchup will be triggered again.")
...
HordeMissionBuffsManager._get_catchup_buff_counts_for_player = function (self, player, waves_completed)
...
HordeMissionBuffsManager._get_num_buffs_for_waves_completed = function (waves_completed, waves_list)
...
HordeMissionBuffsManager._manage_delayed_data_initialization_for_players = function (self)
...
HordeMissionBuffsManager._manage_player_spawn = function (self, player, is_respawn)
...
HordeMissionBuffsManager._request_specific_buff = function (self, buff_name, target_self)
	if target_self then
...
HordeMissionBuffsManager._request_buff_family_choice = function (self, num_choices)
	local is_server_or_host = self:_is_server_or_host()

	if not is_server_or_host then
		Log.error("HordeMissionBuffsManager", "Can only request buff family choice from the server!")
	...
	if not buffs_to_exclude_received_from_backend then
		Log.error("HordeMissionBuffsManager", "No backen list of excluded buffs. Waiting for request.")
...
HordeMissionBuffsManager._request_family_buff_for_all = function (self)
	local is_server_or_host = self:_is_server_or_host()

	if not is_server_or_host then
		Log.error("HordeMissionBuffsManager", "Can only request family buff from the server!")
...
HordeMissionBuffsManager._request_legendary_buff_choice = function (self, wave_num, num_choices)
	local is_server_or_host = self:_is_server_or_host()

	if not is_server_or_host then
		Log.error("HordeMissionBuffsManager", "Can only request legendary buff choice from the server!")
...
HordeMissionBuffsManager.rpc_client_mission_buffs_buff_choices_received = function (self, channel_id, choices_network_id_array, is_buff_family_choice, wave_num)
...
HordeMissionBuffsManager.rpc_client_mission_buffs_buff_received = function (self, channel_id, buff_template_id, wave_num)
	local buff_name = NetworkLookup.buff_templates[buff_template_id]

	Log.info("HordeMissionBuffsManager", "Buff received %s", buff_name)
...
HordeMissionBuffsManager.rpc_client_mission_buffs_family_received = function (self, channel_id, buff_family_id)
	local buff_family_name = NetworkLookup.hordes_build_families[buff_family_id]

	Log.info("HordeMissionBuffsManager", "Buff Family received %s", buff_family_name)

	self._local_player_active_buff_family = buff_family_name
end

HordeMissionBuffsManager._request_buff_for_all = function (self, buff_name)
...
HordeMissionBuffsManager._request_buff_for_self = function (self, buff_name)
	if self:_is_hosting_client() then
		local local_player = Managers.player:local_player(1)
...
HordeMissionBuffsManager._notify_buff_family_given_to_player = function (self, player, buff_family_name)
...
HordeMissionBuffsManager._add_externally_controlled_buff_to_player = function (self, player, buff_name)
...
HordeMissionBuffsManager._notify_server_buff_choice = function (self, choice_index, is_random_selection)
...
HordeMissionBuffsManager._notify_ui = function (context)
	Managers.event:trigger("event_mission_buffs_update_presentation", context)
end

HordeMissionBuffsManager._is_server_or_host = function (self)
	return self._is_server
end

HordeMissionBuffsManager._is_hosting_client = function (self)
	return not DEDICATED_SERVER and self._is_server
end

HordeMissionBuffsManager._is_hosting_player = function (player)
...
HordeMissionBuffsManager._is_client = function (self)
	return not self._is_server
end

HordeMissionBuffsManager._get_local_player_peer_and_local_id = function ()
	local local_player = Managers.player:local_player(1)
...
HordeMissionBuffsManager._convert_buff_name_to_template_id = function (buff_name)
	return NetworkLookup.buff_templates[buff_name]
end

HordeMissionBuffsManager._convert_buff_template_id_to_name = function (buff_template_id)
	return NetworkLookup.buff_templates[buff_template_id]
end

HordeMissionBuffsManager._convert_options_names_array_to_network_ids = function (options_names_array, is_buff_family_choice)
...
HordeMissionBuffsManager._convert_options_network_id_array_to_names = function (buff_template_ids_array, is_buff_family_choice)
	local network_lookup = is_buff_family_choice and NetworkLookup.hordes_build_families or NetworkLookup.buff_templates
...
return HordeMissionBuffsManager
