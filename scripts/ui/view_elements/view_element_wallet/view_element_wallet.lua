...
ViewElementWallet._generate_currencies = function (self, currencies_to_use, widget_size, wallets_per_column, grid_spacing)
	...
	local position = 0
	local max_wallets_per_column = currencies_to_use and #currencies_to_use or table.size(WalletSettings)
	...
	for currency_name, currency_data in pairs(WalletSettings) do
		...
		if currency then
			...
			local grid_position = nil

			if not currencies_to_use then
				local order = WalletSettings[currency].sort_order
				grid_position = {
					math.floor(order / wallets_per_column),
					order % wallets_per_column
				}
			else
				grid_position = {
					math.floor(position / wallets_per_column),
					position % wallets_per_column
				}
			end

			widgets[#widgets + 1] = widget
			widgets_by_currency[currency_name] = widget
		end
	end
...
ViewElementWallet._generate_currency_widget = function (self, currency_name, currency_data, widget_size)
	...
	widget.content.size = widget_size
...
ViewElementWallet._draw_widgets = function (self, dt, t, input_service, ui_renderer, render_settings)
	...
	local updated_wallets_width = {}

	for name, is_updated in pairs(self._wallets_updated) do
		if is_updated then
			recalculate = true
			local widget = self._widgets_by_currency[name]
			...
			updated_wallets_width[widget.name] = widget_width
			self._wallets_updated[name] = false
		end
	end

	if recalculate then
		local widget_width = self._min_width

		for i = 1, #self._wallet_widgets do
			local widget = self._wallet_widgets[i]
			widget_width = math.max(widget_width, widget.content.size[1])
		end
		...
		if self._wallets_per_column == 1 then
			for i = 1, #self._wallet_widgets do
				local widget = self._wallet_widgets[i]
				local grid_position = {
					(i - 1) % self._wallets_per_column,
					math.floor((i - 1) / self._wallets_per_column)
				}
				local widget_size = widget.content.size

				if self._wallets_per_column == #self._wallet_widgets then
					widget.content.size[1] = updated_wallets_width[widget.name]
					widget.offset = {
						total_width,
						0,
						1
					...
					widget.offset = {
						...
						grid_position[2] * widget_size[2] + self._grid_spacing[2] * (i - 1),
						1
					}
				end

				total_width = total_width + widget.content.size[1] + self._grid_spacing[1] * (i - 1)
			end
		else
			for i = 1, #self._wallet_widgets do
				local widget = self._wallet_widgets[i]
		...
		local area_height = math.ceil(#self._wallet_widgets / self._wallets_per_column) * self._widget_height + total_row_spacing
