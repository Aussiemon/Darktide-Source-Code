...
InventoryBackgroundView._setup_top_panel = function (self)
	...
	-- Decompilation error in this vicinity:
	--- BLOCK #3 190-214, warpins: 0 ---
	for i = 1, #self._views_settings, 1 do

		-- Decompilation error in this vicinity:
		--- BLOCK #0 190-195, warpins: 2 ---
	...
	if view_name then
		...
		if active_view == view_name then
			...
			if view_name ~= active_view then
				...
				-- Decompilation error in this vicinity:
				--- BLOCK #1 29-49, warpins: 3 ---
				local context = {}
				context.parent = self
				context.player = self._preview_player
				context.player_level = self._player_level
				context.preview_profile_equipped_items = self._preview_profile_equipped_items
				context.changeable_context = additional_context_data
				context.is_readonly = self._is_readonly
