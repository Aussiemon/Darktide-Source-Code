...
MissionBoardViewLogic.get_ordered_story_missions = function (self, campaign_id)
...
MissionBoardViewLogic.get_campaign_mission_display_order = function (self, mission_key, mission_category, campaign)
	if not mission_key or not mission_category then
	...
	local campaign = campaign or "player-journey"

	return Managers.data_service.mission_board:index_in_campaign("mission", mission_key, mission_category, campaign)
end

MissionBoardViewLogic.get_campaigns_data = function (self)
	return self._campaigns_data
end

MissionBoardViewLogic.get_data_by_campaign = function (self, campaign_id)
	if not campaign_id then
		return nil
	end

	local campaigns_data = self._campaigns_data

	if not campaigns_data then
		return nil
	end

	for _, campaign_data in ipairs(campaigns_data) do
...
MissionBoardViewLogic.is_story_mission = function (self, mission)
	for _, campaign_data in ipairs(self._campaigns_data) do
		for _, campaign_mission in ipairs(campaign_data.missions) do
			if campaign_mission.mission == mission.map and campaign_mission.category == mission.category and table.find(campaign_mission.circumstance, mission.circumstance) and table.find(campaign_mission.missionGiver, mission.missionGiver) then
				return true
			end
		end
	end
...
MissionBoardViewLogic._should_show_mission = function (self, mission)
	...
	if mission.category == "story" then
		if self:_is_story_mission_complete(mission) then
			return false
		elseif mission_data and mission_data.campaign ~= "player-journey" then
			return false
		end
	end
...
MissionBoardViewLogic.get_campaign_mission_progression = function (self)
	local story_missions = self:_get_missions_per_category("story")
	...
	for _, mission in pairs(story_missions) do
...
MissionBoardViewLogic._config_success = function (self, config_results)
	...
	self._ordered_story_missions = self:get_ordered_story_missions()
	self._campaigns_data = Managers.data_service.mission_board:get_campaigns_data()
