...
local MissionBoardViewLogic = require("scripts/ui/views/mission_board_view/mission_board_view_logic")
local MissionBoardViewThemes = require("scripts/ui/views/mission_board_view/mission_board_view_themes")
...
local function _text_height(ui_renderer, text, style, optional_size)
	...
	local height = Text.text_height(ui_renderer, text, style, optional_size)
...
MissionBoardView._get_telemetry_name = function (self)
	local page_index = self._page_index
	local page_settings = self._mission_board_logic:get_current_page(page_index)
	local difficulty_name = page_settings.name or "unknown"
	local category = self:_has_replay_campaign_missions() and "_campaign" or ""

	return string.format("mission_board_view%s_%s", category, difficulty_name)
end

MissionBoardView._update_telemetry_name = function (self, is_closing)
	local new_name = not is_closing and self:_get_telemetry_name() or nil

	if self._telemetry_name == new_name then
		return
	end

	if self._telemetry_name then
	...
	if new_name then
		Managers.telemetry_events:open_view(new_name)

		self._telemetry_name = new_name
	end
end
...
MissionBoardView._open_current_page = function (self)
	...
	if quick_play_settings and not widgets_by_name.qp_mission_widget then
		self:_set_quickplay_widget()
	elseif not quick_play_settings and widgets_by_name.qp_mission_widget then
		local old_widget = widgets_by_name.qp_mission_widget

		self:_remove_mission_widget(old_widget)
	end

	if not widgets_by_name.campaign_playlist_upsell then
		self:_set_static_campaign_playlist_widget()
	end
...
MissionBoardView.on_exit = function (self)
	...
	self:_update_telemetry_name(true)
...
MissionBoardView._set_static_mission_widget = function (self, static_slot_id, widget_id, optional_blueprint_settings, optional_creation_context, optional_on_pressed_callback)
	...
	local slot = self:_claim_slot(static_slot_id)
	...
	local optional_blueprint_settings = optional_blueprint_settings and optional_blueprint_settings or "static_tile"
	local widget = self:_widget_from_blueprint(widget_id, optional_blueprint_settings, nil, creation_context, "mission_id", widget_id, "slot", slot, "static", true)
	...
	local on_pressed_callback = optional_on_pressed_callback or callback(self, "_set_selected", widget_id)
	local position = slot.position
	...
	local was_selected = self._selected_mission_id == widget_id
...
MissionBoardView._set_static_campaign_playlist_widget = function (self)
...
MissionBoardView._set_selected = function (self, id, sidebar_function_name, move_gamepad_cursor, force_reload, ...)
	...
	for _, element in pairs(self._elements) do
		if element.on_mission_selected then
			local mission = id ~= "qp_mission_widget" and self:_mission(id, true) or "qp_mission_widget"

			element:on_mission_selected(mission)
		end
	end
end
...
MissionBoardView._set_next_sidebar_tab = function (self)
	local view_element_mission_objectives_info = self:_element("mission_objectives")

	if view_element_mission_objectives_info then
		view_element_mission_objectives_info:set_next_sidebar_tab()
	end
end

MissionBoardView._has_replay_campaign_missions = function (self)
	local view_element_campaign_mission_list = self:_element("mission_list")

	return view_element_campaign_mission_list and view_element_campaign_mission_list:visible()
end
