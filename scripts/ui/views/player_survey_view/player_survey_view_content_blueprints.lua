...
item_description_text_style.text_color = Color.text_default(255, true)

local option_button_selected_icon_pass = {
	pass_type = "texture",
...
local rating_button_description_label_pass = {
	...
	style = table.add_missing(item_description_text_style, {
...
local blueprints = {
	option_button = {
		...
		pass_template_function = function (self, config, ui_renderer)
			...
			passes.size = nil
			passes[#passes + 1] = option_button_selected_icon_pass
		...
		init = function (parent, widget, element, callback_name, secondary_callback_name, ui_renderer)
			...
			widget.option_id = element.option_id
	...
	rating_button = {
		...
		pass_template_function = function (self, config, ui_renderer)
			...
			passes.size = nil
			passes[#passes + 1] = option_button_selected_icon_pass
			passes[#passes + 1] = rating_button_description_label_pass
		...
		init = function (parent, widget, element, callback_name, secondary_callback_name, ui_renderer)
			...
			widget.option_id = element.option_id
			widget.style.text.font_size = 22
			widget.style.text.text_horizontal_alignment = "center"
			widget.style.text.offset[1] = 10
			widget.style.text.size_addition = {
				-60,
				0,
