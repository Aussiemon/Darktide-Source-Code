...
PlayerSurveyView.open = function (context)
...
PlayerSurveyView.on_enter = function (self)
	...
	local promise = Managers.data_service.player_survey:get_survey(self._context.survey_id)
...
PlayerSurveyView._clean_ui = function (self)
	...
	self._widgets_by_name.question_info_text.content.text = ""
...
PlayerSurveyView._set_question_data = function (self, question_idx)
	...
	if question.type == "option" then
		self:_clean_ui()
		self:_set_option_question_data(question)

		return
	elseif question.type == "rating" then
		self:_clean_ui()
		self:_set_option_question_data(question)
...
PlayerSurveyView._get_option_subtitle_text_formatted = function (question)
	if not question.subtitle then
		return ""
	end

	if question.min > 1 then
		return string.format(question.subtitle, question.min, question.max)
	end

	return string.format(question.subtitle, question.max)
end

PlayerSurveyView._setup_submit_button_for_option = function (self, question, answer_count)
	self._widgets_by_name.submit_button.visible = true
	self._widgets_by_name.submit_button.content.hotspot.disabled = answer_count < question.min
end

PlayerSurveyView._set_option_question_data = function (self, question)
	local option_grid = self:_create_option_grid(question)

	option_grid.question_data = question
	self._widgets_by_name.question_title_text.content.text = question.title

	local subtitle_text = self._get_option_subtitle_text_formatted(question)

	self._widgets_by_name.question_subtitle_text.content.text = subtitle_text

	self:_setup_submit_button_for_option(question, #(self._current_survey_response[question.id] or {}))

	local grid_settings = option_grid:menu_settings()
	...
	for i, v in ipairs(question.options) do
		layout[i] = {
			widget_type = question.type .. "_button",
			text = v.label,
			question = question,
	...
	option_grid:present_grid_layout(layout, PlayerSurveyViewContentBlueprints)

	self._grid = option_grid

	local answer_count = 0

	if self._current_survey_response[question.id] then
		self:_update_option_buttons(question, self._current_survey_response[question.id])

		answer_count = #self._current_survey_response[question.id]
	end

	self:_update_info_text_for_option(question, answer_count)

	if not self:using_cursor_navigation() then
		option_grid:select_first_index()
	end

	option_grid:set_visibility(false)

	self._widgets_by_name.submit_button.visible = false

	Promise.delay(0):next(function ()
		option_grid:set_visibility(true)
...
PlayerSurveyView._cb_on_option_pressed = function (self, widget, is_selected)
	local current_answers = self:_set_current_question_answers(widget.question, widget.option_id, is_selected)

	self:_update_option_buttons(widget.question, current_answers)
	self:_setup_submit_button_for_option(widget.question, #current_answers)
...
PlayerSurveyView._set_current_question_answers = function (self, question, option_id, is_selected)
	...
	if is_selected then
		...
		_remove_option_from_answers(current_answers, option_id)
	end
...
PlayerSurveyView._update_option_buttons = function (self, question, current_answers)
	if question.max == 1 then
		self:_update_option_buttons_single_selection(current_answers)

		return
	end

	self:_update_option_buttons_multi_selection(question, current_answers)
end

PlayerSurveyView._update_option_buttons_single_selection = function (self, current_answers)
	local button_widgets = self._grid:widgets()

	for _, widget in pairs(button_widgets) do
		widget.content.is_selected = table.array_contains(current_answers, widget.option_id)
	end
end

PlayerSurveyView._update_option_buttons_multi_selection = function (self, question, current_answers)
	local option_max_count = question.max
	local button_widgets = self._grid:widgets()
	...
	for _, widget in pairs(button_widgets) do
		...
		if not widget.content.is_selected then
			widget.content.hotspot.disabled = option_max_count <= answer_count
		end
	end
end

PlayerSurveyView._update_info_text_for_option = function (self, question, num_answers)
	local option_count_max = question.max
	local option_count_min = question.min

	if option_count_max == option_count_min and option_count_min == 1 then
	...
	local current_option_color = num_answers == option_count_max and PlayerSurveyViewDefinitions.colors.light_green or PlayerSurveyViewDefinitions.colors.offwhite_green
	local current_option_string = string.format("{#color(%d,%d,%d)}%d{#reset()}", current_option_color[2], current_option_color[3], current_option_color[4], num_answers)
	local max_options_string = string.format("{#color(%d,%d,%d)}/%d{#reset()}", PlayerSurveyViewDefinitions.colors.light_green[2], PlayerSurveyViewDefinitions.colors.light_green[3], PlayerSurveyViewDefinitions.colors.light_green[4], option_count_max)

	self._widgets_by_name.question_info_text.content.text = current_option_string .. max_options_string
end
...
PlayerSurveyView._create_option_grid = function (self, question)
	local option_count = #question.options
	local column_count = 1

	if question.type == "option" then
		column_count = option_count == 3 and 3 or 2
	elseif question.type == "rating" then
		column_count = option_count
	end

	local row_count = math.ceil(option_count / column_count)

	if row_count < 1 then
		row_count = 1
	end

	local grid_height = row_count * PlayerSurveyViewContentBlueprints.option_button.height
...
PlayerSurveyView._upload_survey = function (self)
	...
	local backend_promise = Managers.data_service.player_survey:submit_survey(self._current_data.id, self._current_survey_response)

	Managers.event:trigger("event_start_waiting", backend_promise, LoadingStateData.WAIT_REASON.backend)
	backend_promise:next(function (data)
		Managers.event:trigger("event_add_notification_message", "default", Localize("loc_survey_submit_success"))
	end, function (error)
		Managers.event:trigger("event_add_notification_message", "default", Localize("loc_popup_description_backend_error"))
