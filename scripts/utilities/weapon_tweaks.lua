...
WeaponTweaks.extract_weapon_tweaks = function (path, templates, loaded_files)
	fassert(loaded_files[path] == nil, "Tried to load %q multiple times!", path)
	...
	for name, damage_profile_data in upairs(parent_base_templates) do
		fassert(templates[name] == nil, "Found already existing templates with name %q", name)
	...
	if overrides then
		for name, override in upairs(overrides) do
			fassert(templates[name] == nil, "Found already existing template override with name %q", name)
