-- chunkname: @scripts/utilities/minion_movement.lua
...
MinionMovement.set_anim_driven = function (scratchpad, anim_driven, optional_script_driven_rotation, optional_affected_by_gravity)
	local locomotion_extension = scratchpad.locomotion_extension
...
MinionMovement.get_moving_direction_name = function (unit, scratchpad, optional_destination, optional_rotation)
	...
	local right, forward = Quaternion.right(rotation), Quaternion.forward(rotation)
...
MinionMovement.update_anim_driven_change_target_rotation = function (unit, scratchpad, action_data, t, target_position)
	...
	elseif t >= scratchpad.change_target_rotation_duration then
...
MinionMovement.update_anim_driven_melee_attack_rotation = function (unit, scratchpad, action_data, t, target_position)
	...
	elseif t >= scratchpad.melee_attack_rotation_duration then
...
MinionMovement.update_move_to_ranged_position = function (unit, t, scratchpad, action_data, target_unit, optional_above, optional_below, optional_distance_from_nav_mesh, optional_force_move)
	...
	if t >= scratchpad.move_to_cooldown and (distance_to_destination_sq > scratchpad.max_distance_to_target_sq or force_move_due_to_lost_los) then
		if is_in_idle then
