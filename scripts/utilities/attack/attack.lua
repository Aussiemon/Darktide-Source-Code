...
function _execute(attacked_unit, damage_profile, target_index, target_number, power_level, charge_level, is_critical_strike, dropoff_scalar, attack_direction, instakill, hit_zone_name, hit_world_position, hit_actor, attacking_unit, attacking_unit_owner_unit, apply_owner_buffs, attack_type, herding_template, damage_type, auto_completed_action, item, wounds_shape, triggered_proc_events_or_nil, close_explosion_hit)
	...
	local attacking_unit_owner_data_extension = ScriptUnit.has_extension(attacking_unit_owner_unit, "unit_data_system")
	local attacker_owner_breed_or_nil = attacking_unit_owner_data_extension and attacking_unit_owner_data_extension:breed()
	...
	if target_breed_or_nil then
		hit_weakspot, hit_shield = Weakspot.hit_weakspot(target_breed_or_nil, hit_zone_name, attacking_unit_owner_unit)
	...
	if attacker_stat_buffs then
		local power_stat_buffs = damage_profile.power_stat_buffs

		if power_stat_buffs then
			local stat_multiplier = 1

			for index, stat_buff in pairs(power_stat_buffs) do
				local power_stat_buff = (attacker_stat_buffs[stat_buff] or 1) - 1

				stat_multiplier = stat_multiplier + power_stat_buff
			end

			power_level = power_level * stat_multiplier
		end
	end
	...
	if instakill then
		...
		if not damage_immune and has_random_damage_immunity then
			local target_stat_buffs = target_buff_extension and target_buff_extension:stat_buffs()
			local random_damage_immunity_chance = target_stat_buffs and target_stat_buffs.random_damage_immunity_chance or 0
			local random_damage_immune = has_random_damage_immunity and random_damage_immunity_chance > math.random()
		...
		if damage_immune then
			...
			local stagger_impact_bonus = 1 + (havoc_extension and havoc_extension:get_modifier_value("stagger_impact_bonus") or 0)

			calculated_damage, damage_efficiency = DamageCalculation.calculate(damage_profile, damage_type, target_settings, damage_profile_lerp_values, hit_zone_name, power_level * power_level_damage_multiplier, charge_level, target_breed_or_nil, attacker_owner_breed_or_nil, attacker_breed_or_nil, is_critical_strike, hit_weakspot, hit_shield, effective_backstab, effective_flanking, dropoff_scalar, attack_type, attacker_stat_buffs, target_stat_buffs, attacker_buff_extension, target_buff_extension, armor_penetrating, target_health_extension, target_toughness_extension, armor_type, target_stagger_count, num_triggered_staggers, is_attacked_unit_suppressed, distance, attacked_unit, auto_completed_action, current_stagger_impact, stagger_impact_bonus, attacking_unit, attacking_unit_owner_unit, attacker_owner_buff_extension, target_index)
		end
	end
	...
	if was_alive_at_attack_start and target_breed_or_nil then
		...
		if is_server then
			...
			_record_stats(attack_result, attack_type, attacked_unit, attacking_unit_owner_unit, damage_absorbed, damage_dealt, hit_zone_name, damage_profile, item, attacked_action, attacker_owner_breed_or_nil, target_breed_or_nil, damage_type, attacker_owner_buff_extension, target_buff_extension, hit_weakspot, is_backstab, is_critical_strike, stagger_result, stagger_type)
			_record_telemetry(attacking_unit_owner_unit, attacked_unit, attack_result, attack_type, damage_dealt, damage_profile, damage_type, damage, permanent_damage, actual_damage_dealt, damage_absorbed, attacker_owner_breed_or_nil, target_breed_or_nil, instakill)
		end
	...
	if was_alive_at_attack_start and attack_result == attack_results.died then
		_trigger_elite_special_kill_interfacing(attacking_unit_owner_unit, attacker_owner_breed_or_nil, target_breed_or_nil, is_server)
	end

	if damage_dealt <= 0 and not is_player_character and is_server then
	...
	local attacker_is_player_character = Breed.is_player(attacker_owner_breed_or_nil)
...
function _handle_buffs(is_server, triggered_proc_events_or_nil, damage_profile, attacker_owner_buff_extension_or_nil, target_buff_extension_or_nil, attacked_unit, damage, damage_absorbed, actual_damage_dealt, attack_result, stagger_result, was_staggered_before_attack, hit_zone_name, is_critical_strike, is_backstab, hit_weakspot, one_hit_kill, attack_type, attacking_unit, attacking_owner_unit, attack_direction, damage_efficiency, target_index, target_number, attacker_owner_breed_or_nil, attacker_breed_or_nil, target_breed_or_nil, damage_type, charge_level, hit_world_position_or_nil, attacking_item_or_nil, close_explosion_hit, instakill)
	...
	if attacker_is_player and target_is_ally or not attacker_owner_buff_extension_or_nil and not target_buff_extension_or_nil then
		return
	end
...
function _record_stats(attack_result, attack_type, attacked_unit, attacking_unit, damage_absorbed, damage_dealt, hit_zone_name, damage_profile, attacking_item, attacked_action, attacker_owner_breed_or_nil, target_breed_or_nil, damage_type, attacker_owner_buff_extension, target_buff_extension, hit_weakspot, is_backstab, is_critical_hit, stagger_result, stagger_type)
	...
	if attacking_player then
		...
		_attack_table.hit_weakspot = hit_weakspot
...
function _record_telemetry(attacking_unit, attacked_unit, attack_result, attack_type, damage_dealt, damage_profile, damage_type, damage, permanent_damage, actual_damage_dealt, damage_absorbed, attacker_owner_breed_or_nil, target_breed_or_nil, instakill)
	...
	if attacked_player then
		data.attacker = attacker_owner_breed_or_nil and attacker_owner_breed_or_nil.name
		data.is_boss = attacker_owner_breed_or_nil and attacker_owner_breed_or_nil.is_boss
...
function _handle_result(attacking_unit_owner_unit, attacked_unit, attack_result, attack_type, attacker_owner_breed_or_nil, target_breed_or_nil, damage_dealt, damage_absorbed, damage_profile, damage_type, actual_damage_dealt)
	if not attacking_unit_owner_unit then
		return
	end
...
function _trigger_elite_special_kill_interfacing(attacking_unit, attacker_owner_breed_or_nil, target_breed_or_nil, is_server)
