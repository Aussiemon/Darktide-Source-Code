...
Interrupt.action = function (t, unit, reason, reason_data_or_nil, ignore_immunity)
	...
	if sprinting and action_settings then
		local allowed_during_sprint = ActionAvailability.available_in_sprint(action_settings)
		local sprint_requires_press_to_interrupt = Sprint.requires_press_to_interrupt(action_settings)
