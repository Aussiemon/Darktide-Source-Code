-- chunkname: @scripts/utilities/companion_follow.lua

local Blackboard = require("scripts/extension_systems/blackboard/utilities/blackboard")
local NavQueries = require("scripts/utilities/nav_queries")
local Sprint = require("scripts/extension_systems/character_state_machine/character_states/utilities/sprint")
local CompanionFollowUtility = {}
local default_nav_distances = {
	4,
	4,
	0.25,
	0,
...
local default_new_positions = {}
local default_new_positions_after_check = {}
local default_current_scratchpad = {}
...
CompanionFollowUtility.companion_has_follow_position = function (unit, blackboard, scratchpad, condition_args, action_data, is_running, dt)
	...
	if current_owner_cooldown == -1 then
		...
		if not Sprint.is_in_start_slowdown(behavior_component.owner_unit, character_state_component, sprint_character_state_component) then
...
CompanionFollowUtility.calculate_flrb_position = function (unit, scratchpad, dt, velocity, action_data)
	...
	if math.radians_to_degrees(angle_between_velocities) > scratchpad.max_angle_per_check then
		...
		move_to_position, current_direction, new_position_calculated = CompanionFollowUtility._calculate_new_flrb_position(owner_unit, unit, new_stored_velocity, scratchpad.follow_config, scratchpad.locomotion_extension:current_velocity(), scratchpad, action_data)
	else
		follow_component.last_referenced_vector:store(velocity)

		move_to_position, current_direction, new_position_calculated = CompanionFollowUtility._calculate_new_flrb_position(owner_unit, unit, velocity, scratchpad.follow_config, scratchpad.locomotion_extension:current_velocity(), scratchpad, action_data)
	end
...
CompanionFollowUtility._calculate_new_flrb_position = function (owner_unit, companion_unit, current_velocity, follow_config, companion_velocity, scratchpad, action_data)
	...
	while true do
		...
		if not has_left_hit or not has_right_hit then
			local new_positions_after_check = CompanionFollowUtility._look_for_valid_position(owner_unit, action_data, scratchpad, not has_left_hit, not has_right_hit, left_position, right_position, default_nav_distances)
	...
	local is_left_valid = not has_left_hit
	local is_right_valid = not has_right_hit
	local new_positions_after_check = CompanionFollowUtility._look_for_valid_position(owner_unit, action_data, scratchpad, is_left_valid, is_right_valid, left_position, right_position, default_nav_distances)
	...
	if size > 0 then
		if size > 1 then
			...
			follow_left_direction = companion_distance_left_position <= companion_distance_right_position
		else
		...
		return nav_new_position, follow_left_direction, true
	else
		return nil, nil, false
	end
end
...
CompanionFollowUtility.calculate_follow_owner_position = function (unit, blackboard, scratchpad, condition_args, action_data, is_running, dt)
	...
	local new_positions_after_check = CompanionFollowUtility._look_for_valid_position(owner_unit, action_data, scratchpad, not has_front_hit, not has_back_hit, front_position, back_position, default_nav_distances)
	local size = table.size(new_positions_after_check)
...
CompanionFollowUtility._calculate_position_around_owner = function (unit, scratchpad, action_data)
	...
	valid_positions, valid_prioritized_positions = CompanionFollowUtility.select_points_around_center_with_owner_forward(unit, scratchpad, action_data, owner_position, distance_from_owner, default_nav_distances, false, 1)
...
CompanionFollowUtility.select_points_around_another_center = function (unit, scratchpad, action_data, center, forward_vector, nav_distances, stop_at_first, debug_draw_timer)
	table.clear(default_valid_positions)
	table.clear(valid_prioritized_positions)
	...
	for i = 1, angle_checks do
		...
		if new_position then
			...
			if ray_can_go then
				table.insert(default_valid_positions, new_position)
			end
		end
		...
		if stop_at_first and new_position then
			return default_valid_positions, valid_prioritized_positions
		end
	end
...
CompanionFollowUtility._look_for_valid_position = function (owner_unit, action_data, scratchpad, is_first_valid, is_second_valid, first_position, second_position, nav_distances)
	...
	table.clear(default_new_positions)

	if is_both_valid then
		default_new_positions.first_position = first_position
		default_new_positions.second_position = second_position
	...
	table.clear(default_new_positions_after_check)

	for key, position in pairs(default_new_positions) do
		local new_position = default_new_positions[key]
		local nav_world, traverse_logic = scratchpad.navigation_extension:nav_world(), scratchpad.navigation_extension:traverse_logic()
		...
		if nav_new_position then
			default_new_positions_after_check[key] = nav_new_position
		end
	end

	return default_new_positions_after_check
end
...
CompanionFollowUtility._create_scratchpad_follow_flrb = function (unit, blackboard, scratchpad, condition_args, action_data, is_running, dt)
	table.clear(default_current_scratchpad)

	local behavior_component = Blackboard.write_component(blackboard, "behavior")

	default_current_scratchpad.behavior_component = behavior_component

	local follow_component = Blackboard.write_component(blackboard, "follow")

	default_current_scratchpad.follow_component = follow_component

	local navigation_extension = ScriptUnit.extension(unit, "navigation_system")

	default_current_scratchpad.animation_extension = ScriptUnit.extension(unit, "animation_system")
	default_current_scratchpad.locomotion_extension = ScriptUnit.extension(unit, "locomotion_system")
	default_current_scratchpad.navigation_extension = navigation_extension

	local aim_component = Blackboard.write_component(blackboard, "aim")

	default_current_scratchpad.aim_component = aim_component

	local spawn_component = Blackboard.write_component(blackboard, "spawn")

	default_current_scratchpad.spawn_component = spawn_component

	local owner_unit = behavior_component.owner_unit

	default_current_scratchpad.owner_unit = owner_unit
	default_current_scratchpad.owner_locomotion_extension = ScriptUnit.extension(owner_unit, "locomotion_system")
	default_current_scratchpad.current_direction_timer = 0

	return default_current_scratchpad
end
