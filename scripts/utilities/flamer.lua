...
Flamer.start_aiming_fx = function (t, unit, vfx, sfx, wwise_world, world, flamer_data)
	local attachment_unit = flamer_data.attachment_unit
	local source_node_index = flamer_data.source_node_index
	local start_position = _get_start_position(attachment_unit, source_node_index)
	...
	if sfx then
		...
		if aim_sfx_event then
			...
			flamer_data.muzzle_source_id = muzzle_source_id
		end
	end
end

Flamer.start_shooting_fx = function (t, unit, vfx, sfx, wwise_world, world, flamer_data)
	local attachment_unit = flamer_data.attachment_unit
	local source_node_index = flamer_data.source_node_index

	assert(attachment_unit, "no attachment unit supplied")
	assert(source_node_index, "no source_node_index supplied")
	...
	if vfx then
		...
		if muzzle_particle_name then
			...
			World.link_particles(world, muzzle_particle_id, attachment_unit, source_node_index, Matrix4x4.identity(), orphaned_policy)

			flamer_data.muzzle_particle_id = muzzle_particle_id
		end
		...
		if flamer_particle_name then
			...
			World.link_particles(world, flamer_particle_id, attachment_unit, source_node_index, Matrix4x4.identity(), orphaned_policy)

			flamer_data.flamer_particle_id = flamer_particle_id
			...
			if num_parabola_control_points then
				...
				flamer_data.parabola_particle_variables = parabola_particle_variables
			end
		end
	end
	...
	if sfx then
		...
		if looping_sfx_start_event then
			if not flamer_data.muzzle_source_id then
				flamer_data.muzzle_source_id = WwiseWorld.make_manual_source(wwise_world, start_position, Quaternion.identity())
			end

			local muzzle_source_id = flamer_data.muzzle_source_id

			WwiseWorld.trigger_resource_event(wwise_world, looping_sfx_start_event, muzzle_source_id)
		end
	end

	flamer_data.start_t = t
end

Flamer.update_shooting_fx = function (t, unit, vfx, sfx, wwise_world, world, physics_world, aim_position, control_point_1, control_point_2, flamer_data)
	local attachment_unit = flamer_data.attachment_unit
	local source_node_index = flamer_data.source_node_index
	local start_position = _get_start_position(attachment_unit, source_node_index)
	local direction = Vector3.normalize(aim_position - start_position)
	local range = flamer_data.range
	...
	if vfx.hit_dynamic_particle then
		local dynamic_hit_particle_id = flamer_data.dynamic_hit_particle_id
		...
		if hit_dynamic then
			...
			if dynamic_hit_particle_id then
				...
				flamer_data.dynamic_hit_particle_id = World.create_particles(world, vfx.hit_dynamic_particle, dynamic_hit_position, directional_rotation)
			end
		elseif dynamic_hit_particle_id then
			World.stop_spawning_particles(world, dynamic_hit_particle_id)

			flamer_data.dynamic_hit_particle_id = nil
		end
	end
	...
	if ground_impact_particle_name then
		...
		local ground_impact_particle_id = flamer_data.ground_impact_particle_id
		...
		if hit_ground then
			...
			if ground_impact_particle_id then
				...
				flamer_data.ground_impact_particle_id = World.create_particles(world, ground_impact_particle_name, ground_hit_position, tangent_rotation)
			end
		elseif ground_impact_particle_id then
			World.stop_spawning_particles(world, ground_impact_particle_id)

			flamer_data.ground_impact_particle_id = nil
		end
	end

	local parabola_particle_variables = flamer_data.parabola_particle_variables

	if parabola_particle_variables then
		local flamer_particle_id = flamer_data.flamer_particle_id

		_set_flamer_parabola(world, attachment_unit, source_node_index, flamer_particle_id, parabola_particle_variables, start_position, control_point_1, control_point_2, aim_position)
	end
...
Flamer.stop_fx = function (unit, vfx, sfx, wwise_world, world, flamer_data)
	if sfx then
		...
		if looping_sfx_stop_event then
			...
			if muzzle_source_id then
				...
				flamer_data.muzzle_source_id = nil
			end
		end
	end
	...
	if vfx then
		...
		local muzzle_particle_id = flamer_data.muzzle_particle_id

		if muzzle_particle_id then
			World.stop_spawning_particles(world, muzzle_particle_id)

			flamer_data.muzzle_particle_id = nil
		end
		...
		if flamer_particle_id then
			...
			flamer_data.flamer_particle_id = nil
		end
	end
end

function _get_start_position(attachment_unit, source_node_index)
	local start_position = Unit.world_position(attachment_unit, source_node_index)
