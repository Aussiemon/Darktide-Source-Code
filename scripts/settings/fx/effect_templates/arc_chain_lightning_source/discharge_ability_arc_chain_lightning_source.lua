...
local function _on_chain_node_add_func(node, context)
	...
	if context.is_server then
		...
		if target_buff_extension then
			target_buff_extension:add_internally_controlled_buff("discharge_arc_electrocution", start_t, "owner_unit", context.player_unit, "source_item", context.source_item)
