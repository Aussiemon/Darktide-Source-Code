-- chunkname: @scripts/game_states/boot/boot_state_load_render_settings.lua

require("scripts/game_states/boot/state_boot_sub_state_base")

local BootStateLoadRenderSettings = class("BootStateLoadRenderSettings", "StateBootSubStateBase")
...
BootStateLoadRenderSettings._state_update = function (self, dt)
	if not IS_WINDOWS or LEVEL_EDITOR_TEST or DEDICATED_SERVER then
		return true
	end
...
BootStateLoadRenderSettings._verify_disabled_status = function (self, settings)
	if not DEDICATED_SERVER then
...
return BootStateLoadRenderSettings
