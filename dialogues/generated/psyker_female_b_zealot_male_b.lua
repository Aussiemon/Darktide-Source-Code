-- chunkname: @dialogues/generated/psyker_female_b_zealot_male_b.lua

local psyker_female_b_zealot_male_b = {
	bonding_conversation_metropolitan_dram_b = {
		randomize_indexes_n = 0,
		sound_events_n = 1,
		sound_events = {
			[1] = "loc_zealot_male_b__bonding_conversation_metropolitan_dram_b_01",
		},
		sound_events_duration = {
			[1] = 6.676938,
		},
		randomize_indexes = {},
	},
	bonding_conversation_metropolitan_dram_d = {
		randomize_indexes_n = 0,
		sound_events_n = 1,
		sound_events = {
			[1] = "loc_zealot_male_b__bonding_conversation_metropolitan_dram_d_01",
		},
		sound_events_duration = {
			[1] = 1.988833,
		},
		randomize_indexes = {},
	},
	bonding_conversation_metropolitan_figment_b = {
		randomize_indexes_n = 0,
		sound_events_n = 1,
		sound_events = {
			[1] = "loc_zealot_male_b__bonding_conversation_metropolitan_figment_b_01",
		},
		sound_events_duration = {
			[1] = 2.660354,
		},
		randomize_indexes = {},
	},
	bonding_conversation_metropolitan_frown_b = {
		randomize_indexes_n = 0,
		sound_events_n = 1,
		sound_events = {
			[1] = "loc_zealot_male_b__bonding_conversation_metropolitan_frown_b_01",
		},
		sound_events_duration = {
			[1] = 7.295542,
		},
		randomize_indexes = {},
	},
	bonding_conversation_metropolitan_frown_d = {
		randomize_indexes_n = 0,
		sound_events_n = 1,
		sound_events = {
			[1] = "loc_zealot_male_b__bonding_conversation_metropolitan_frown_d_01",
		},
		sound_events_duration = {
			[1] = 1.56725,
		},
		randomize_indexes = {},
	},
	bonding_conversation_metropolitan_stories_b = {
		randomize_indexes_n = 0,
		sound_events_n = 5,
		sound_events = {
			"loc_zealot_male_b__bonding_conversation_metropolitan_stories_b_01",
			"loc_zealot_male_b__bonding_conversation_metropolitan_stories_b_02",
			"loc_zealot_male_b__bonding_conversation_metropolitan_stories_b_03",
			"loc_zealot_male_b__bonding_conversation_metropolitan_stories_b_04",
			"loc_zealot_male_b__bonding_conversation_metropolitan_stories_b_05",
		},
		sound_events_duration = {
			4.874688,
			5.308021,
			6.961688,
			6.022938,
			6.311479,
		},
		randomize_indexes = {},
	},
	bonding_conversation_metropolitan_stories_d = {
		randomize_indexes_n = 0,
		sound_events_n = 3,
		sound_events = {
			"loc_zealot_male_b__bonding_conversation_metropolitan_stories_d_01",
			"loc_zealot_male_b__bonding_conversation_metropolitan_stories_d_02",
			"loc_zealot_male_b__bonding_conversation_metropolitan_stories_d_03",
		},
		sound_events_duration = {
			2.282229,
			4.011521,
			1.792604,
		},
		randomize_indexes = {},
	},
	oval_bonding_conversation_wishes_b = {
		randomize_indexes_n = 0,
		sound_events_n = 1,
		sound_events = {
			[1] = "loc_zealot_male_b__oval_bonding_conversation_wishes_b_01",
		},
		sound_events_duration = {
			[1] = 3.186521,
		},
		randomize_indexes = {},
	},
	oval_bonding_conversation_wishes_d = {
		randomize_indexes_n = 0,
		sound_events_n = 1,
		sound_events = {
			[1] = "loc_zealot_male_b__oval_bonding_conversation_wishes_d_01",
		},
		sound_events_duration = {
			[1] = 4.828063,
		},
		randomize_indexes = {},
	},
	oval_world_conversation_cumbernauld_brahms_seventeen_b = {
		randomize_indexes_n = 0,
		sound_events_n = 1,
		sound_events = {
			[1] = "loc_zealot_male_b__oval_world_conversation_cumbernauld_brahms_seventeen_b_01",
		},
		sound_events_duration = {
			[1] = 4.951,
		},
		randomize_indexes = {},
	},
	oval_world_conversation_cumbernauld_brahms_seventeen_d = {
		randomize_indexes_n = 0,
		sound_events_n = 1,
		sound_events = {
			[1] = "loc_zealot_male_b__oval_world_conversation_cumbernauld_brahms_seventeen_d_01",
		},
		sound_events_duration = {
			[1] = 1.458292,
		},
		randomize_indexes = {},
	},
}

return settings("psyker_female_b_zealot_male_b", psyker_female_b_zealot_male_b)
