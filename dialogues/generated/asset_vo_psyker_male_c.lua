-- chunkname: @dialogues/generated/asset_vo_psyker_male_c.lua

local asset_vo_psyker_male_c = {
	asset_acid_clouds = {
		randomize_indexes_n = 0,
		sound_events_n = 2,
		sound_events = {
			[1] = "loc_psyker_male_c__zone_watertown_acid_clouds_01",
			[2] = "loc_psyker_male_c__zone_watertown_acid_clouds_02"
		},
		sound_events_duration = {
			[1] = 4.404333,
			[2] = 4.133458
		},
		sound_event_weights = {
			[1] = 0.5,
			[2] = 0.5
		},
		randomize_indexes = {}
	},
	asset_cartel_insignia = {
		randomize_indexes_n = 0,
		sound_events_n = 2,
		sound_events = {
			[1] = "loc_psyker_male_c__mission_cartel_insignia_01",
			[2] = "loc_psyker_male_c__mission_cartel_insignia_02"
		},
		sound_events_duration = {
			[1] = 5.26999,
			[2] = 3.178729
		},
		sound_event_weights = {
			[1] = 0.5,
			[2] = 0.5
		},
		randomize_indexes = {}
	},
	asset_foul_smoke = {
		randomize_indexes_n = 0,
		sound_events_n = 4,
		sound_events = {
			"loc_psyker_male_c__asset_foul_smoke_01",
			"loc_psyker_male_c__asset_foul_smoke_02",
			"loc_psyker_male_c__asset_foul_smoke_03",
			"loc_psyker_male_c__asset_foul_smoke_04"
		},
		sound_events_duration = {
			6.360344,
			2.605229,
			1.838198,
			2.400354
		},
		randomize_indexes = {}
	},
	asset_goo = {
		randomize_indexes_n = 0,
		sound_events_n = 2,
		sound_events = {
			[1] = "loc_psyker_male_c__level_hab_block_goo_01",
			[2] = "loc_psyker_male_c__level_hab_block_goo_02"
		},
		sound_events_duration = {
			[1] = 3.48301,
			[2] = 5.288646
		},
		sound_event_weights = {
			[1] = 0.5,
			[2] = 0.5
		},
		randomize_indexes = {}
	},
	asset_grease_pit = {
		randomize_indexes_n = 0,
		sound_events_n = 2,
		sound_events = {
			[1] = "loc_psyker_male_c__asset_grease_pit_01",
			[2] = "loc_psyker_male_c__asset_grease_pit_02"
		},
		sound_events_duration = {
			[1] = 1.347375,
			[2] = 1.078469
		},
		randomize_indexes = {}
	},
	asset_sigil = {
		randomize_indexes_n = 0,
		sound_events_n = 2,
		sound_events = {
			[1] = "loc_psyker_male_c__asset_sigil_01",
			[2] = "loc_psyker_male_c__asset_sigil_02"
		},
		sound_events_duration = {
			[1] = 2.713323,
			[2] = 3.986698
		},
		randomize_indexes = {}
	},
	asset_unnatural_dark_a = {
		randomize_indexes_n = 0,
		sound_events_n = 4,
		sound_events = {
			"loc_psyker_male_c__asset_unnatural_dark_a_01",
			"loc_psyker_male_c__asset_unnatural_dark_a_02",
			"loc_psyker_male_c__asset_unnatural_dark_a_03",
			"loc_psyker_male_c__asset_unnatural_dark_a_04"
		},
		sound_events_duration = {
			1.215177,
			0.928219,
			2.326302,
			2.075563
		},
		randomize_indexes = {}
	},
	asset_unnatural_dark_b = {
		randomize_indexes_n = 0,
		sound_events_n = 4,
		sound_events = {
			"loc_psyker_male_c__asset_unnatural_dark_b_01",
			"loc_psyker_male_c__asset_unnatural_dark_b_02",
			"loc_psyker_male_c__asset_unnatural_dark_b_03",
			"loc_psyker_male_c__asset_unnatural_dark_b_04"
		},
		sound_events_duration = {
			0.766906,
			1.340417,
			1.594292,
			1.421031
		},
		randomize_indexes = {}
	},
	asset_water_course = {
		randomize_indexes_n = 0,
		sound_events_n = 2,
		sound_events = {
			[1] = "loc_psyker_male_c__mission_cartel_water_course_01",
			[2] = "loc_psyker_male_c__mission_cartel_water_course_02"
		},
		sound_events_duration = {
			[1] = 3.307875,
			[2] = 3.492
		},
		sound_event_weights = {
			[1] = 0.5,
			[2] = 0.5
		},
		randomize_indexes = {}
	}
}

return settings("asset_vo_psyker_male_c", asset_vo_psyker_male_c)
